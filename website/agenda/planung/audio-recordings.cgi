...
sub delete_recording {
    ...
    ConfigError->throw(error => "Cannot delete audio file '$file', file does not exist\n")
        unless -e $file;
    delete_file($file);
    local $config->{access}->{write} = 1;
...
sub show_audio_recording {
    ...
    my $days = 24 * 60 * 60;
    $params->{error} = "show is more than a week ahead" if ($now + 7 * $days) < $start;
    ...
    for my $cmd (sort values %{$config->{"audio-upload-hooks"}}) {
        ...
        while (defined (my $line = (<$fh>))) {
            if ($line =~ m/^calcms_audio_recordings\.([a-zA-Z0-9_-]+)\s*=\s*(\S+)/) {
                my ($key, $value) = ($1, $2);
                $entry->{$key} = $value;
                DbError->throw(error => "invalid column $key for table calcms_audio_recordings")
                    unless exists audio_recordings::get_columns($config)->{$key};
                audio_recordings::update($config, $entry);
            } elsif ($line =~ m/^calcms_events\.([a-zA-Z0-9_-]+)\s*=\s*(\S+)/) {
                my ($key, $value) = ($1, $2);
                DbError->throw(error => "invalid column $key for calcms_events\n")
