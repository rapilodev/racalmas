...
sub main {
    ...
    return show_active_users($config, $request) if $params->{action} eq 'show-active-users';
    return show_user_stats( $config, $request) if $params->{action} eq 'show-user-stats';
}
