...
my ($user, $expires) = try {
    ...
    auth::show_login_form('', $_->message)
        ;    # if ref $_;# and $_->isa('AuthError') or $_->isa('SessionError');
    ...
    )
{
    ...
    $headerParams->{loc} =
        localization::get($config, { user => $user, file => 'menu' });
    template::process($config, 'print',
...
sub showCalendar {
    ...
    print user_settings::getColorCss($config,
    ...
    if ($params->{list} == 1) {
        ...
        if ($params->{part} == 1) {
            calcCalendarTable($config, $permissions, $params, $calendar,
...
sub printToolbar {
        ...
        )
    {
        ...
        $toolbar .=
              qq{<option value="$filter">}
            . $params->{loc}->{ 'label_' . $key }
            . '</option>';
    }
