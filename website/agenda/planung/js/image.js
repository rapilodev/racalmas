...
class ImageManager {
    ...
    prepareUpload() {
        ...
        if (saveBtn) {
            saveBtn.innerHTML = "";
            saveBtn.append(
                this._btn("upload", loc.button_upload_image || "Upload", () => this.triggerFilePicker(), "primary"),
    ...
    async loadImageEditor(filename) {
        ...
        if (saveBtn) {
            saveBtn.innerHTML = "";
            saveBtn.append(this._btn("save", loc.button_save_image || "Save", () => this.saveImage(this.getFormData())));
        }
    }
    ...
    renderTools(image) {
        ...
        if (image.public == 1 || image.public === "1") {
            ...
            tools.append(assign_btn,
                this._btn("private", loc.button_depublish_image, () => this.togglePublish(image, 0))
            );
        } else if (image.licence?.trim().length > 0) {
            let publish_button = this._btn("public", loc.button_publish_image, () => this.togglePublish(image, 1));
            publish_button.classList.add("primary");
            tools.append(publish_button);
        }

        if (this.permissions.create_image && !this.isUploadMode) {
            tools.append(this._btn("upload", loc.button_upload_image || "Upload New Image", () => this.prepareUpload(), "width-full"));
        }

        if (this.permissions.delete_image_others) {
            let btn = this._btn("delete", loc.button_delete_image || "Delete", () => {
