...
sub save_schedule {
    ...
    return uac::permissions_denied('update_schedule')
        unless $permissions->{update_schedule} == 1;

    return uac::print_error("$_ not given") for
    ...
    return uac::print_error('no period type selected!') if $found == 0;
    ...
    if (($entry->{period_type} eq 'single') && ($entry->{exclude} eq '1')) {
        return uac::permissions_denied('delete_schedule')
...
sub delete_series {
    ...
    my $params      = $request->{params}->{checked};
    my $permissions = $request->{permissions};
    return uac::permissions_denied('delete_series')
...
sub save_series {
    ...
    return uac::print_error("please set at least series name!")
    ...
    if ($params->{action} eq 'create') {
        ...
        return uac::permissions_denied('insert, entry already exists') if scalar(@$series_ids) > 0;
        ...
        return uac::print_error('could not insert series') unless defined $series_id;
    } elsif ($params->{action} eq 'save') {

        return uac::permissions_denied('update_series') unless $permissions->{update_series} == 1;
        return uac::permissions_denied('update. missing parameter series_id')
        ...
        return uac::permissions_denied(q{update due to series already exists multiple times with name "$entry->{series_name}" and title "$entry->{title}"})
            if scalar(@$series_ids) > 1;
        return uac::permissions_denied('update due to series id does not match to existing entry')
        ...
        return uac::print_error('could not update series') unless defined $result;
    }
...
sub assign_event {
    ...
    if (scalar(@$events) != 1) {
        uac::print_error("no event found for event_id=$entry->{event_id}, phase=all");
        return undef;
    }
...
sub add_user {
    ...
    for my $param ('project_id', 'studio_id', 'series_id', 'user_id') {
        return uac::print_error("missing $param") if $params->{$param} eq '';
    }
...
sub remove_user {
    ...
    my $params      = $request->{params}->{checked};
    my $permissions = $request->{permissions};
    return uac::permissions_denied('remove_series_member')
...
sub list_series {
    ...
    $params->{image} =
      studios::getImageById($config, { uac::set($params, 'project_id', 'studio_id') })
...
sub show_series {
    ...
    my $params      = $request->{params}->{checked};
    my $permissions = $request->{permissions};
    return uac::permissions_denied('read_series')
        unless $permissions->{read_series} == 1;

    for my $param ('project_id', 'studio_id', 'series_id') {
        return uac::print_error("missing $param")
    ...
    if (@$series > 1) {
        return uac::print_error(
            "too much series found for studio '" . $studio_by_id->{$params->{studio_id}}->{name} . "'");
    }

    if (@$series == 0) {
        return uac::print_error("selected series not assigned to studio '"
...
sub set_rebuilt_episodes {
    ...
    my $params      = $request->{params}->{checked};
    my $permissions = $request->{permissions};
    return uac::permissions_denied('read_series') unless $permissions->{read_series} == 1;
