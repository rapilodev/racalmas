...
use Exception::Class ('ParamError', 'PermissionError', 'DateTimeError');
...
sub main {
    ...
    if ($params->{action} =~ m/^(show_series|list_series)$/ && !params::isJson()) {
        ...
        $out .= template::process($config,
                template::check($config, 'series-header.html'),
                $headerParams);
        $out .= template::process($config,
                template::check($config, 'show-series-header.html'), {})
            if $params->{action} eq 'show_series';
        $out .= template::process($config,
                template::check($config, 'list-series-header.html'), {})
            if $params->{action} eq 'list_series';
    }
    ...
    return $out . show_series($config, $request)
        if $params->{action} eq 'show_series';
    return $out . list_series($config, $request)
        if $params->{action} eq 'list_series';

    if ($params->{action} eq 'save_schedule') {
        my $result = $out.save_schedule($config, $request);
        print STDERR Dumper ("output:",$result);
        print $result;return;
        return $result;
    }
    return $out . save_schedule($config, $request)
        if $params->{action} eq 'save_schedule';
    return $out . delete_schedule($config, $request)
        if $params->{action} eq 'delete_schedule';
    return $out . add_user($config, $request)
        if $params->{action} eq 'add_user';
    return $out . remove_user($config, $request)
        if $params->{action} eq 'remove_user';
    return $out . save_series($config, $request)
        if $params->{action} eq 'save_series';
    return $out . save_series($config, $request)
        if $params->{action} eq 'create_series';
    return $out . delete_series($config, $request)
        if $params->{action} eq 'delete_series';
    return $out . assign_event($config, $request)
        if $params->{action} eq 'assign_event';
    return $out . unassign_event($config, $request)
        if $params->{action} eq 'unassign_event';
    return $out . reassign_event($config, $request)
        if $params->{action} eq 'reassign_event';
    return $out . rebuild_episodes($config, $request)
        if $params->{action} eq 'rebuild_episodes';
    return $out . set_rebuilt_episodes($config, $request)
...
sub save_series {
    ...
    my $series_ids = series::get(
        $config,
        {   uac::set(
                $entry, 'project_id', 'studio_id', 'series_id', 'title'
            ),
        }
    );
    ...
    if ($params->{action} eq 'create_series') {
        ...
        InsertError->throw(error => 'could not insert series')
...
sub assign_event {
    ...
    PermissionError->throw(
        error => "missing permission to assign_series_events")
...
sub reassign_event {
    ...
    for my $attr (
        'project_id', 'studio_id', 'series_id', 'new_series_id',
        'event_id'
        )
    {
        ParamError->throw(error => "missing $attr at reassign_event")
            unless defined $params->{$attr};
    }
